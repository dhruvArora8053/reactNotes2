...
function MovieDetails({ selectedId, onCloseMovie, onAddWatched, watched }) {
  ...
  // so we created this ref here where we want to store the amount of clicks that happened on the rating before the movie is added but we don't want to render that information onto the usere interface on in other words we don't want to create a re-render, so that's why ref is perfect for this

  // let count = 0;
  // this won't work becuse it is not persistent, it's value will always get initialised after each render
  // we have state which does both of these things so triggers a re-render and it is persistent, and then in the middle we have a ref which is indeed persisted across renders but it does not trigger a re-render when updated so that's why we don't usually use ref in the jsx output
  ...
  // now each time the useRating was updated the component was re-rendered and so then after that re-render, this effect was executed  which means that after the rating had been updated then our ref would be updated as well, so we would update the current property to simply adding 1
